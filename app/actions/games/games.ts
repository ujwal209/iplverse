...
export async function getMatchClues(matchId: number) {
  try {
    ...
    try {
      ...
      // Key rotation retry loop for Tavily
      let context = "";
      let tavOk = false;
      let tavAttempts = 0;
      const maxTavAttempts = TAVILY_API_KEYS.length;

      while (!tavOk && tavAttempts < maxTavAttempts) {
        ...
        try {
          ...
          if (tavRes.ok) {
            ...
            context = tavData.results?.map((r: any) => r.content).join(" ") || "No context found.";
            tavOk = true;
          } else {
            console.warn(`Tavily key fail status ${tavRes.status}. Retrying next key...`);
          }
        } catch (err) {
          console.warn(`Tavily call exception:`, err);
        }
      }
...
Respond ONLY with a raw JSON object (no markdown, no backticks) in exactly this format:
{
  ...
  "funFact": "Your fun fact here"
        ...
        // Key rotation retry loop for Groq
        let groqOk = false;
        let groqAttempts = 0;
        const maxGroqAttempts = GROQ_API_KEYS.length;

        while (!groqOk && groqAttempts < maxGroqAttempts) {
          ...
          try {
            ...
            if (groqRes.ok) {
              ...
              console.warn(`Groq key fail status ${groqRes.status}. Retrying next key...`);
            }
          } catch (err) {
            console.warn(`Groq call exception:`, err);
          }
        }
      }
    } catch (apiErr) {
...
function cleanObject(obj: any): any {
  ...
  if (Array.isArray(obj)) {
    const cleanedArr = obj.map(v => cleanObject(v)).filter(v => v !== undefined);
...
export async function getPlayerAIHints(playerName: string, dbStats?: any) {
  try {
    const cleanedStats = dbStats ? cleanObject(dbStats) : undefined;
    ...
    while (!tavOk && tavAttempts < maxTavAttempts) {
      ...
      try {
        ...
        if (tavRes.ok) {
          ...
          console.warn(`Tavily key fail status ${tavRes.status}. Retrying key...`);
        }
      } catch (err) {
        console.warn(`Tavily call exception:`, err);
      }
...
We are querying the player "${playerName}".
${cleanedStats ? `Here are the official database statistics for this player (ONLY use non-null values):
${JSON.stringify(cleanedStats, null, 2)}` : ""}
...
${cleanedStats ? `- Make sure all clues are strictly consistent with the official database stats. Do not contradict the database stats.` : ""}
      ...
      while (!groqOk && groqAttempts < maxGroqAttempts) {
        ...
        try {
          ...
          if (groqRes.ok) {
            ...
            console.warn(`Groq key fail status ${groqRes.status}. Retrying key...`);
          }
        } catch (err) {
          console.warn(`Groq call exception:`, err);
        }
...
all their IPL franchises in chronological order. only ipl franchise
Ensure there are at least 4 entries if possible. Make it detailed.
prefer the ones who have changed teams man not in one team only
...
Format:
{
  "journey": [
    { "year": "YYYY", "team": "Team Name" }
        ...
        console.error("AI Journey generation error on attempt:", apiErr);
      }
    }
    
    if (journey.length === 0) {
      return { success: false, error: "Failed to generate AI career journey from Groq." };
    }
    
    // Group consecutive team years to avoid duplicates (e.g. 2008, 2009, 2010 -> 2008-2010)
    const normalize = (t: string) => {
      let n = t.toLowerCase().trim();
      if (n.startsWith("rising pune")) return "rising pune supergiant";
      if (n.startsWith("royal challengers")) return "royal challengers bangalore";
      return n;
    };

    const groupedJourney: { year: string; team: string }[] = [];
    if (journey.length > 0) {
      let current = { ...journey[0] };
      for (let i = 1; i < journey.length; i++) {
        const entry = journey[i];
        if (normalize(entry.team) === normalize(current.team)) {
          const startYear = current.year.split(/[-–—]/)[0].trim();
          const endYear = entry.year.split(/[-–—]/).pop()?.trim() || entry.year.trim();
          current.year = startYear === endYear ? startYear : `${startYear}-${endYear}`;
        } else {
          groupedJourney.push(current);
          current = { ...entry };
        }
      }
      groupedJourney.push(current);
