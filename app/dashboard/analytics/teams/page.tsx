...
import { TeamAutocomplete } from "@/components/dashboard/team-autocomplete";
...
export default function TeamsHub() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (!team.trim()) return;
    router.push(`/dashboard/analytics/teams/${encodeURIComponent(team)}`);
  };
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 space-y-12 max-w-5xl mx-auto pt-12 pb-24">
      <div className="text-center space-y-4 w-full">
        ...
        <h1 className="text-4xl md:text-5xl font-bold outfit-bold tracking-tight">Franchise Analytics</h1>
        <p className="inter-regular text-muted-foreground text-lg max-w-2xl mx-auto">
          Explore historical franchise performance, win rates, and title counts.
        </p>
      </div>
      ...
      <div className="w-full max-w-2xl mx-auto mt-8 bg-card border border-border shadow-2xl rounded-3xl p-6 sm:p-10">
        <form onSubmit={handleSearch} className="flex flex-col gap-6 items-center justify-center">
          <TeamAutocomplete 
            placeholder="Search IPL Franchise..."
            value={team}
            onChange={setTeam}
          />
